interface LogoProps {
  /** 'light' for dark backgrounds (sidebar), 'dark' for white backgrounds (landing page) */
  variant?: 'light' | 'dark';
  className?: string;
 ...
 * Standardized to DevSpark Architecture (No hardcoded hex colors, uses tailwind config)
 */
export const Logo = ({
  variant = 'dark',
  className = '',
  ...
  // Tailwind classes based on the variant
  const isLight = variant === 'light';
  const iconColorClass = isLight
    ? 'fill-surface-DEFAULT stroke-surface-DEFAULT'
    : 'fill-slate-800 stroke-slate-800';
  const mainTextColorClass = isLight ? 'text-surface-DEFAULT' : 'text-slate-800';
  ...
  return (
    <div className={`flex items-center gap-0 select-none ${className}`}>
      ...
      <svg
        ...
        // Applying the standard Tailwind icon sizing and the dynamic color classes
        className={`shrink-0 ${iconClassName} ${iconColorClass}`}
