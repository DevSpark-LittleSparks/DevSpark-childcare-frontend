...
import { 
  MdDashboard, MdChecklist, MdAssignment, MdRestaurant, 
  MdChat, MdPerson, MdSettings, MdLogout, MdKeyboardArrowUp,
  MdMenu, MdMenuOpen 
} from "react-icons/md";
...
const LittleSparksLogo = ({ isCollapsed }: { isCollapsed: boolean }) => (
  <div className={`flex items-center gap-0 select-none ${isCollapsed ? 'justify-center' : ''}`}>
    <svg version="1.0" xmlns="http://www.w3.org/2000/svg" className="shrink-0 w-10 h-10" viewBox="0 0 260.000000 280.000000" preserveAspectRatio="xMidYMid meet">
      <g transform="translate(0.000000,280.000000) scale(0.100000,-0.100000)" className="fill-slate-800 stroke-slate-800 dark:fill-white dark:stroke-white" strokeWidth="80" strokeLinecap="round" strokeLinejoin="round">
...
export default function TeacherSidebar({ isCollapsed, setIsCollapsed, onOpenSettings }: SidebarProps) {
  ...
  return (
    <aside className={`h-screen bg-[#E4F7F7] dark:bg-slate-900 dark:bg-slate-900 flex flex-col fixed left-0 top-0 z-[1000] border-r border-cyan-100 dark:border-slate-800 shadow-sm transition-all duration-300 ${isCollapsed ? 'w-[80px]' : 'w-[280px]'}`}>
      
      {/* Brand & Toggle Section */}
      <div className={`p-4 flex items-center border-b border-cyan-200/30 dark:border-slate-800/50 mb-2 ${isCollapsed ? 'flex-col gap-4' : 'justify-between px-6'}`}>
      ...
      <nav className="flex-1 px-3 py-2 overflow-y-auto scrollbar-hide overflow-x-hidden">
        <NavGroup title="My Classroom" isCollapsed={isCollapsed}>
          <SidebarLink to="/teacher/dashboard" icon={<MdDashboard />} label="Home" isCollapsed={isCollapsed} />
          <SidebarLink to="/teacher/attendance" icon={<MdChecklist />} label="Daily Attendance" isCollapsed={isCollapsed} />
          <SidebarLink to="/teacher/activities" icon={<MdAssignment />} label="My Activities" isCollapsed={isCollapsed} />
          <SidebarLink to="/teacher/meals" icon={<MdRestaurant />} label="Meals" isCollapsed={isCollapsed} />
        </NavGroup>

        <NavGroup title="Communication" isCollapsed={isCollapsed}>
          <SidebarLink to="/teacher/messages" icon={<MdChat />} label="Messaging" isCollapsed={isCollapsed} />
        </NavGroup>
      </nav>
      ...
      <div className="p-4 border-t border-cyan-200/30 dark:border-slate-800/50 relative" ref={menuRef}>
        {open && (
          <div className={`absolute bottom-[85px] ${isCollapsed ? 'left-16 w-48' : 'left-4 right-4'} bg-white dark:bg-slate-800 rounded-2xl p-2 shadow-2xl border border-slate-100 dark:border-slate-700 z-[1001]`}>
            <button className="w-full flex items-center gap-3 p-3 text-sm font-bold text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/50 rounded-xl" onClick={() => { setOpen(false); navigate("/teacher/profile"); }}>
              <MdPerson className="text-xl text-slate-400" /> Profile
            </button>
            <button className="w-full flex items-center gap-3 p-3 text-sm font-bold text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/50 rounded-xl" onClick={() => { setOpen(false); onOpenSettings?.(); }}>
              <MdSettings className="text-xl text-slate-400" /> Settings
            </button>
            <div className="h-[1px] bg-slate-100 dark:bg-slate-700 my-1"></div>
            <button className="w-full flex items-center gap-3 p-3 text-sm font-bold text-red-500 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-xl" onClick={handleSignOut}>
              <MdLogout className="text-xl" /> Sign out
            </button>
          </div>
        )}
        <div className={`flex items-center gap-3 p-2 rounded-2xl cursor-pointer transition-all border ${open ? 'bg-white dark:bg-slate-800 shadow-md border-cyan-200 dark:border-slate-700' : 'hover:bg-white/50 dark:hover:bg-slate-800/50 border-transparent'} ${isCollapsed ? 'justify-center' : ''}`} onClick={() => setOpen(!open)}>
          {user?.photoURL ? (
            <img 
              src={user?.photoURL} 
            ...
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-primary-400 to-indigo-500 flex items-center justify-center text-white font-black text-lg shrink-0 shadow-sm border-2 border-white dark:border-slate-700">
              {user?.displayName ? user.displayName.charAt(0).toUpperCase() : 'T'}
            </div>
          )}
          {!isCollapsed && (
            <div className="flex-1 min-w-0">
              <p className="text-sm font-bold text-slate-800 dark:text-white truncate">{user?.displayName || "Teacher"}</p>
              <p className="text-[10px] font-bold text-cyan-700 dark:text-cyan-400 uppercase tracking-wider">{user?.role || "Educator"}</p>
            </div>
          )}
...
const SidebarLink = ({ to, icon, label, isCollapsed }: { to: string, icon: any, label: string, isCollapsed: boolean }) => (
  <NavLink 
    ...
    className={({ isActive }) => `flex items-center gap-4 px-4 py-3 rounded-xl text-sm font-bold transition-all group relative ${
      isActive ? "bg-[#CFFAFE] dark:bg-cyan-900/40 text-[#0891B2] dark:text-cyan-400 shadow-sm" : "text-slate-500 dark:text-slate-400 hover:bg-white/60 dark:hover:bg-slate-800/50"
