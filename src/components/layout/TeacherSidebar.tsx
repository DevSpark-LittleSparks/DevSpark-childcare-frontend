  ...
  return (
    <aside className="w-[280px] h-screen bg-[#E4F7F7] flex flex-col fixed left-0 top-0 z-[1000] border-r border-cyan-100 shadow-sm">
      ...
      <nav className="flex-1 px-4 py-2 overflow-y-auto scrollbar-hide">
        <div className="pt-2">
          <p className="px-4 text-[10px] font-black text-cyan-600/60 uppercase tracking-[0.15em] mb-2">My Classroom</p>
          <div className="space-y-1">
            <SidebarLink to="/teacher/dashboard" icon={<MdDashboard />} label="Home" />
            <SidebarLink to="/teacher/attendance" icon={<MdChecklist />} label="Daily Attendance" />
            <SidebarLink to="/teacher/activities" icon={<MdAssignment />} label="My Activities" />
            <SidebarLink to="/teacher/meals" icon={<MdRestaurant />} label="Meals" />
          </div>
        </div>

        <div className="pt-6">
          <p className="px-4 text-[10px] font-black text-cyan-600/60 uppercase tracking-[0.15em] mb-2">Communication</p>
          <SidebarLink to="/teacher/messages" icon={<MdChat />} label="Messaging" />
        </div>
      </nav>
      ...
      <div className="p-4 border-t border-cyan-200/30 relative" ref={menuRef}>
        {open && (
          <div className="absolute bottom-[85px] left-4 right-4 bg-white rounded-2xl p-2 shadow-2xl border border-slate-100 z-[1001]">
            <button className="w-full flex items-center gap-3 p-3 text-sm font-bold text-slate-600 hover:bg-slate-50 rounded-xl" onClick={() => navigate("/teacher/profile")}>
        ...
        <div className={`flex items-center gap-3 p-3 rounded-2xl cursor-pointer transition-all border ${open ? 'bg-white shadow-md border-cyan-200' : 'hover:bg-white/50 border-transparent'}`} onClick={() => setOpen(!open)}>
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-bold text-slate-800 truncate">{user?.firstName || "Teacher"}</p>
            <p className="text-[10px] font-bold text-cyan-600 uppercase">Educator</p>
          </div>
