...
import { 
  MdDashboard, MdPeople, MdTrendingUp, MdPayment, 
...
export default function ParentSidebar({ onOpenSettings }: { onOpenSettings?: () => void }) {
  ...
  return (
    <aside className="w-[280px] h-screen bg-[#E4F7F7] flex flex-col fixed left-0 top-0 z-[1000] border-r border-cyan-100 shadow-sm">
      <div className="p-6 border-b border-cyan-200/30 mb-2">
        <LittleSparksLogo />
      </div>

      <nav className="flex-1 px-4 py-2 overflow-y-auto scrollbar-hide">
        <NavLink to="/parent/dashboard" className={({ isActive }) => `flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-bold transition-all mb-4 ${isActive ? "bg-[#CFFAFE] text-[#0891B2] shadow-sm" : "text-slate-500 hover:bg-white/60"}`}>
          <MdDashboard className="text-xl" /> <span>Home</span>
        </NavLink>

        <div className="pt-2">
          <p className="px-4 text-[10px] font-black text-cyan-600/60 uppercase tracking-[0.15em] mb-2">Parent Portal</p>
          <div className="space-y-1">
            <SidebarLink to="/parent/children" icon={<MdPeople />} label="My Children" />
            <SidebarLink to="/parent/progress" icon={<MdTrendingUp />} label="Progress" />
            <SidebarLink to="/parent/payments" icon={<MdPayment />} label="Payments" />
          </div>
        </div>

        <div className="pt-6">
          <p className="px-4 text-[10px] font-black text-cyan-600/60 uppercase tracking-[0.15em] mb-2">Communication</p>
          <SidebarLink to="/parent/messages" icon={<MdChat />} label="Messaging" />
        </div>
      </nav>

      <div className="p-4 border-t border-cyan-200/30 relative" ref={menuRef}>
        {open && (
          <div className="absolute bottom-[85px] left-4 right-4 bg-white rounded-2xl p-2 shadow-2xl border border-slate-100 z-[1001]">
            <button className="w-full flex items-center gap-3 p-3 text-sm font-bold text-slate-600 hover:bg-slate-50 rounded-xl" onClick={() => navigate("/parent/profile")}>
              <MdPerson className="text-xl text-slate-400" /> Profile
            </button>
            <button className="w-full flex items-center gap-3 p-3 text-sm font-bold text-slate-600 hover:bg-slate-50 rounded-xl" onClick={onOpenSettings}>
        ...
        <div className={`flex items-center gap-3 p-3 rounded-2xl cursor-pointer transition-all border ${open ? 'bg-white shadow-md border-cyan-200' : 'hover:bg-white/50 border-transparent'}`} onClick={() => setOpen(!open)}>
          <img src={user?.avatarUrl || `https://ui-avatars.com/api/?name=${user?.firstName}&background=06B6D4&color=fff`} className="w-10 h-10 rounded-xl object-cover border-2 border-white" alt="Profile" />
          <div className="flex-1 min-w-0">
            <p className="text-sm font-bold text-slate-800 truncate">{user?.firstName || "Parent"}</p>
            <p className="text-[10px] font-bold text-cyan-600 uppercase">Guardian</p>
          </div>
          <MdKeyboardArrowUp className={`text-slate-400 transition-transform ${open ? "rotate-180" : ""}`} />
        </div>
      </div>
    </aside>
  );
...
const SidebarLink = ({ to, icon, label }: { to: string, icon: any, label: string }) => (
  <NavLink to={to} className={({ isActive }) => `flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-bold transition-all ${isActive ? "bg-[#CFFAFE] text-[#0891B2]" : "text-slate-500 hover:bg-white/60"}`}>
