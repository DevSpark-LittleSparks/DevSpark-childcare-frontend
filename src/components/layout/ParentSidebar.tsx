...
export default function ParentSidebar({ isCollapsed, setIsCollapsed, onOpenSettings }: SidebarProps) {
  ...
  return (
    <aside className={`h-screen bg-[#E4F7F7] dark:bg-slate-900 flex flex-col fixed left-0 top-0 z-[1000] border-r border-cyan-100 dark:border-slate-800 shadow-sm transition-all duration-300 ${isCollapsed ? 'w-[80px]' : 'w-[280px]'}`}>
      ...
      <div className="p-4 border-t border-cyan-200/30 dark:border-slate-800/50 relative" ref={menuRef}>
        {open && (
          <div className={`absolute bottom-[85px] ${isCollapsed ? 'left-16 w-48' : 'left-4 right-4'} bg-white dark:bg-slate-800 rounded-2xl p-2 shadow-2xl border border-slate-100 dark:border-slate-700 z-[1001]`}>
            <button className="w-full flex items-center gap-3 p-3 text-sm font-bold text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/50 rounded-xl" onClick={() => { setOpen(false); navigate("/parent/profile"); }}>
            ...
            <button className="w-full flex items-center gap-3 p-3 text-sm font-bold text-red-500 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-xl" onClick={handleSignOut}>
        ...
        <div className={`flex items-center gap-3 p-2 rounded-2xl cursor-pointer transition-all border ${open ? 'bg-white dark:bg-slate-800 shadow-md border-cyan-200 dark:border-slate-700' : 'hover:bg-white/50 dark:hover:bg-slate-800/50 border-transparent'} ${isCollapsed ? 'justify-center' : ''}`} onClick={() => setOpen(!open)}>
          {user?.photoURL ? (
            ...
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-indigo-400 to-purple-600 flex items-center justify-center text-white font-black text-lg shrink-0 shadow-sm border-2 border-white dark:border-slate-700">
              {user?.displayName ? user.displayName.charAt(0).toUpperCase() : 'P'}
            </div>
          )}
          {!isCollapsed && (
            <div className="flex-1 min-w-0">
              <p className="text-sm font-bold text-slate-800 dark:text-white truncate">{user?.displayName || "Parent"}</p>
              <p className="text-[10px] font-bold text-cyan-700 dark:text-cyan-400 uppercase tracking-wider">{user?.role || "Guardian"}</p>
            </div>
          )}
