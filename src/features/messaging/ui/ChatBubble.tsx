    ...
    <div className={cn('flex w-full mb-6', isCurrentUser ? 'justify-end' : 'justify-start')}>
      ...
      <div
        className={cn(
          ...
          isCurrentUser ? 'bg-chat-bubbleRight text-white rounded-tr-sm' : 'bg-white text-gray-800 rounded-tl-sm'
        )}
      >
        {message.content === '...' ? (
        ...
        >
          ...
          {isCurrentUser && message.isRead && <span className="ml-1 text-white text-[10px]">✓✓</span>}
        </div>
      </div>
    </div>
  );
