...
import {
  User, Lock, Mail, Phone, MapPin, Baby, Eye, EyeOff,
...
const ParentProfilePage: React.FC<ParentProfilePageProps> = ({ initialUser }) => {
  ...
  const generateSuggestion = () => {
    ...
    const suggested = `SPARK-${user.name.split(' ')[0].toUpperCase() || 'PARENT'}-${randomID}`;
          ...
          <div className="pt-28 p-10 space-y-12">
            ...
            <div className="flex justify-between items-center bg-slate-50/50 p-4 rounded-[2rem] border border-slate-100">
              <div className="flex items-center gap-3 ml-2">
                ...
                <h2 className="text-slate-500 text-[10px] font-black uppercase tracking-[0.3em]">Parent Profile Management</h2>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-10">
              ...
              <div className="space-y-3 group">
                ...
                <div className="relative">
                  <User className={`absolute left-4 top-4 transition-colors ${!isEditing ? 'text-slate-300' : 'text-slate-400 group-focus-within:text-primary-500'}`} size={18} />
                  <input
                    name="name"
                    value={user.name}
                    ...
                    className={`w-full pl-12 p-4 rounded-2xl border-2 transition-all font-bold text-sm ${
                      ...
                      : 'bg-white border-slate-100 text-slate-700 focus:border-primary-500 focus:ring-4 focus:ring-primary-500/5 shadow-sm'
                    }`}
                  />
                </div>
              </div>
              ...
              {[
                ...
                <div key={field.name} className="space-y-3 group">
                  ...
                  <div className="relative">
                    <field.icon className={`absolute left-4 top-4 transition-colors ${field.disabled ? 'text-slate-300' : 'text-slate-400 group-focus-within:text-primary-500'}`} size={18} />
                    <input
                      name={field.name}
              ...
              <div className="md:col-span-2 space-y-3">
                ...
                <div className="relative">
                  ...
                  <textarea
                    ...
                    onChange={handleInputChange}
                    disabled={!isEditing}
                    className="w-full pl-12 p-5 bg-white border-2 border-slate-100 rounded-[1.8rem] focus:border-primary-500 focus:ring-4 focus:ring-primary-500/5 transition-all disabled:bg-slate-50 font-bold text-sm shadow-sm"
                    rows={3}
                  />
                </div>
              </div>
            ...
            {/* My Little Sparks - Only for Parents */}
            <div className="bg-slate-50/50 rounded-[2.5rem] p-8 border border-slate-100 relative overflow-hidden">
              <div className="absolute top-0 right-0 p-8 opacity-5">
                 <Baby size={120} className="text-primary-500" />
              </div>
              <h3 className="text-xs font-black text-slate-800 mb-8 flex items-center gap-3 uppercase tracking-[0.2em] relative z-10">
                 Enrolled <span className="text-primary-500 underline decoration-primary-500/30 underline-offset-8">Little Sparks</span>
              </h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 relative z-10">
                {user.children?.map((child) => (
                  <div key={child.id} className="flex items-center justify-between p-6 bg-white border border-slate-100 rounded-[2rem] hover:shadow-2xl hover:shadow-primary-500/5 hover:border-primary-500/30 transition-all cursor-pointer group">
                    <div className="flex items-center gap-5">
                      <div className="bg-slate-900 h-14 w-14 rounded-2xl flex items-center justify-center text-primary-500 font-black group-hover:scale-105 group-hover:rotate-3 transition-transform duration-300 shadow-lg shadow-black/10">
                        {child.name.charAt(0)}
                      </div>
                      <div>
                        <p className="font-black text-slate-800 text-base tracking-tight">{child.name}</p>
                    ...
                    <div className="h-10 w-10 rounded-2xl bg-slate-50 flex items-center justify-center text-slate-400 group-hover:bg-primary-500 group-hover:text-white transition-all duration-300">
                      <Edit2 size={16} />
                    </div>
                  </div>
                ))}
