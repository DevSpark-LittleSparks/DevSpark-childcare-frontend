  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (validateForm()) {
      ...
      try {
        const payload = {
          ...formData,
          // The form keeps these as strings, but the backend DTO types them
          // as Double - send real numbers (or null when left blank).
      ...
      <header className="max-w-7xl mx-auto px-6 pt-8 pb-4">
        <div className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-6">
            ...
            >
              <ArrowLeft
                className="text-slate-400 dark:text-slate-500 dark:text-slate-400 group-hover:text-primary-500"
                size={20}
              />
            </button>
          </div>
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-8 pb-20">
          ...
          <div className="lg:col-span-2 space-y-8">
            <div className="bg-white dark:bg-[#0f172a] rounded-[3rem] shadow-[0_20px_60px_rgba(10,6,55,0.03)] border border-slate-100 dark:border-slate-800/60 dark:border-slate-800/60 p-8 md:p-12">
              <div className="flex items-center gap-4 mb-10">
                ...
                <h2 className="text-xl font-black text-midnight dark:text-white uppercase tracking-tight">
                  Student Information
                </h2>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-8 mb-10">
                <div className="flex flex-col items-center">
                  ...
                  <input
                    ...
                    accept="image/*"
                  />
                  <p className="mt-3 text-[9px] font-black text-slate-400 dark:text-slate-500 dark:text-slate-400 uppercase tracking-widest text-center">
                    Child Photo
                  </p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-8">
                <div className="space-y-2 text-left">
                  <label className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest ml-1 flex items-center gap-1">
                    <Droplets size={10} /> Blood Group
                  </label>
                  <select
                    name="bloodGroup"
                    onChange={handleInputChange}
                    className={`w-full p-4 bg-slate-100 dark:bg-slate-800/40 rounded-2xl outline-none text-sm font-bold appearance-none border-2 ${errors.bloodGroup ? 'border-red-500' : 'border-transparent focus:border-primary-500'}`}
                  >
                ...
                <div className="space-y-2 text-left lg:col-span-2">
                  ...
                  <input
                    ...
                    onChange={handleInputChange}
                    className={`w-full p-4 bg-slate-100 dark:bg-slate-800/40 border-2 ${errors.address ? 'border-red-500' : 'border-transparent focus:border-primary-500'} rounded-2xl outline-none text-sm font-bold`}
                  />
                ...
                <div className="space-y-2 text-left">
                  ...
                  <input
                    ...
                    placeholder="110"
                    onChange={handleInputChange}
                    className={`w-full p-4 bg-slate-100 dark:bg-slate-800/40 border-2 ${errors.height ? 'border-red-500' : 'border-transparent focus:border-primary-500'} rounded-2xl outline-none text-sm font-bold`}
                  />
                ...
                <div className="space-y-2 text-left">
                  ...
                  <input
                    ...
                    placeholder="20"
                    onChange={handleInputChange}
                    className={`w-full p-4 bg-slate-100 dark:bg-slate-800/40 border-2 ${errors.weight ? 'border-red-500' : 'border-transparent focus:border-primary-500'} rounded-2xl outline-none text-sm font-bold`}
                  />
          ...
          <div className="space-y-8 animate-fadeUp" style={{ animationDelay: '0.2s' }}>
            <div className="bg-midnight rounded-[3rem] p-10 shadow-2xl relative overflow-hidden text-white">
              ...
              <div
                className="absolute -bottom-32 -left-32 w-64 h-64 bg-indigo-500/20 rounded-full blur-[80px] animate-pulse"
              ...
              <div className="space-y-5 text-left relative z-10">
                <div className="space-y-2 text-left">
                  ...
                  >
                    ...
                    <option value="father" className="bg-midnight">
                      Father
                    </option>
                    <option value="mother" className="bg-midnight">
                      Mother
                    </option>
                    <option value="guardian" className="bg-midnight">
                      Guardian
                    </option>
                  </select>
...
const LittleInput = ({ label, dark, error, ...props }: any) => (
  <div className="space-y-2 text-left">
    <label
      className={`text-[10px] font-black uppercase tracking-widest ml-1 ${dark ? 'text-slate-400 dark:text-slate-500 dark:text-slate-400' : 'text-slate-500 dark:text-slate-400'}`}
    >
      {label}
    </label>
    <input
