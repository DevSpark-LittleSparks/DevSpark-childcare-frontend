...
const LittleSparksLogo = () => (
  <div className="flex items-center gap-0 select-none">
    <svg version="1.0" xmlns="http://www.w3.org/2000/svg" className="shrink-0 w-8 h-8" viewBox="0 0 260.000000 280.000000" preserveAspectRatio="xMidYMid meet">
    ...
    <span 
      className="font-bold tracking-tighter -ml-1 text-xl text-[#1F2937]" 
...
export const AuthHeader: React.FC<AuthHeaderProps> = ({ backLink = "/login" }) => {
  return (
    <header className="w-full flex items-center justify-between px-6 py-4 bg-white/80 backdrop-blur-sm border-b border-slate-100 sticky top-0 z-[50]">
      {/* Logo Wrapper */}
      ...
      <Link 
        to={backLink} 
        className="flex items-center text-sm font-bold text-slate-500 hover:text-cyan-600 transition-colors group"
      >
